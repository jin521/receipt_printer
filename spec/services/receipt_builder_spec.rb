...
RSpec.describe ReceiptBuilder do
  ...
  describe '.build' do
    it 'builds a receipt from CSV file' do
      ...
      expect(product1.price).to eq(12.49)
      ...
      expect(product2.price).to eq(14.99)

      expect(receipt_builder.sales_tax).to be_nil
      expect(receipt_builder.total).to be_nil

      receipt_builder.calculate_totals
